...
def _parse_time_anchor(text: str) -> Optional[int]:
    """Parses single time anchors like 'noon', 'midnight', '3 PM', '14:00'."""
...
def extract_hours_window(text: str) -> Optional[List[int]]:
    ...
    # Single hour match: "at 3 PM", "during hour 15"
...
def parse_operator_note_fallback(
    ...
    if has_solar and hours and any(w in lower for w in ["reduc", "curtail", "cut", "drop", "wash", "clean", "offline", "forecast", "haze", "cloud", "storm", "dust"]):
        factor = 0.0
        if "offline" in lower or "no solar" in lower or "zero solar" in lower:
            ...
            if m_remain:
                ...
                # Check "reduced by X%" or "X% reduction" or "cut by X%"
    ...
    negation_keywords = ["no ", "don't", "do not", "stop", "prevent", "forbid", "prohibit", "disable", "disabled", "maintenance", "freeze", "unavailable", "isolated", "offline"]
    has_charge = any(k in lower for k in charge_keywords)
    has_neg = any(k in lower for k in negation_keywords)

    if has_charge and has_neg and hours and "discharge" not in lower:
        return {
            "note_index": note_index,
            "applies": True,
    ...
    discharge_keywords = ["discharge", "discharging", "draw from storage", "draw power from the battery", "freeze battery", "drain"]
    ...
    reserve_keywords = ["reserve", "minimum", "at least", "no lower than", "maintain", "keep >= ", "keep at or above", "hold", "remain in the battery"]
    ...
    # 5. Max Grid Window
    has_grid = any(k in lower for k in ["grid", "import", "utility", "intake", "transformer", "feeder"])
    has_limit = any(k in lower for k in ["cap", "limit", "max", "no more than", "exceed", "below", "at or below", "restriction", "constrained", "is "])

    if (has_grid and has_limit) and hours:
        m_grid = re.search(r"(?:exceed|below|cap|limit|at or below|stay at or below|to|is)\s+(\d+(?:\.\d+)?)\s*(?:kwh|kw)?", lower)
        if not m_grid:
